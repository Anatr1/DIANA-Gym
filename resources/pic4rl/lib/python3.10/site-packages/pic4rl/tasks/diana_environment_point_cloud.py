...
# Porting of pic4rl_environment_lidar to stable baselines3
...
class DIANAEnvironmentPointCloud(gym.Env): # L'originale importa da Node
     # Added stuff
    subscriptions = ["lidar", "goal_info", "robot_pose", "collision"]

    def __init__(self):
        """
        """
        super(DIANAEnvironmentPointCloud, self).__init__()
    ...
    # method called to take an action with the environment, it returns the next observation, the immediate reward,
    # whether the new state is a terminal state, whether the max number of timesteps is reached, and additional information
    def step(self, action, episode_step=0):
        ...
        return observation, reward, done, info #MANCA TERMINATED/TRUNCATED
    ...
    # This method should return an observation AND a dictionary with additional info (default empty)
    def reset(self, n_episode, tot_steps, evaluate=False):
        ...
        return observation #MANCA IL DIZIONARIO
    ...
    def create_subscription(self, topic, msg_type, callback, qos_profile):
        pass

rclpy.init()
env = DIANAEnvironmentPointCloud()
check_env(env)