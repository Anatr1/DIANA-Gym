...
#from .pic4rl_environment_lidar import Pic4rlEnvironmentLidar
from .diana_environment_point_cloud import DIANAEnvironmentPointCloud
#from .pic4rl_environment_camera_depth import Pic4rlEnvironmentCamera
...
class DIANATrainingPointCloud(DIANAEnvironmentPointCloud):
    def __init__(self):
        super().__init__()
        self.log_check()
    ...
    def instantiate_agent(self):
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent()")
        action =[
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() action: ", action)
        low_action = []
        high_action = []
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() low_action: ", low_action)
        low_action = np.array(low_action, dtype=np.float32)
        high_action = np.array(high_action, dtype=np.float32)

        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() high_action: ", high_action)
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() self.action_space: ", self.action_space)
        state =[
        [0., 15.], # goal_distance 
        [-math.pi, math.pi], # goal angle or yaw
        ]

        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() state: ", state)
        for i in range(self.lidar_points):
            state = state + [[0., 12.]]

        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() state2: ", state)
        if len(state)>0:
            low_state = []
            high_state = []
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() self.low_state: ", self.low_state)
        ...
        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() self.observation_space: ", self.observation_space)
        #self.print_log()

        print("DEBUG: DIANATrainingPointCloud: instantiate_agent() self.train_policy: ", self.train_policy)
        print("DEBUG: OFF-POLICY ALGORITHM TRAINER")
        # OFF-POLICY ALGORITHM TRAINER
        if self.policy_trainer == 'off-policy':
            print("DEBUG: off-policy")
    ...
    def threadFunc(self):
        try:
            print("DEBUG: Starting trainer...")
            self.trainer() # ORIGINAL
            #self.instantiate_agent()
            #self.trainer = self.instantiate_agent()
            print("DEBUG: Trainer finished.")
        except Exception:
